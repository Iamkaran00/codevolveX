...
import { getAllCourses, fetchCourseCategories } from "../services/operations/courseApi";
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex items-center gap-1">
        {Array.from({ length: 5 }, (_, i) => (
          <svg key={i} width="16" height="16" viewBox="0 0 12 12">
            <path
              d="M6 1l1.236 2.507L10 3.934l-2 1.95.472 2.755L6 7.25l-2.472 1.39L4 5.884l-2-1.95 2.764-.427L6 1z"
              fill={i < full ? "#fbbf24" : "#e2e8f0"}
            />
          </svg>
        ))}
      </div>
      <div className="flex items-baseline gap-1.5 ml-1">
        {value > 0 && (
          <span className="text-base font-bold text-slate-900">{value.toFixed(1)}</span>
        )}
        <span className="text-sm font-medium text-slate-500">({count.toLocaleString()} reviews)</span>
      </div>
    </div>
  );
}
    ...
    >
      <div className="relative w-full aspect-[16/9] overflow-hidden bg-slate-50 shrink-0">
        ...
        <div className="absolute top-4 left-4 flex flex-wrap gap-2">
          <span className="bg-white/95 backdrop-blur-sm text-slate-800 text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-full shadow-sm">
            {category}
          </span>
          {level && (
      ...
      <div className="flex flex-col flex-grow p-7">
        <h3 className="text-xl font-bold text-slate-900 line-clamp-2 leading-tight group-hover:text-indigo-600 transition-colors duration-300">
          {course.courseName}
        </h3>

        <p className="text-base text-slate-500 mt-2">
          Instructed by <span className="font-semibold text-slate-700">{instructorName}</span>
        </p>
  ...
  return (
    ...
    >
      ...
      <p className="text-slate-500 mt-3 text-lg max-w-md">
        We couldn't find anything matching your criteria. Try adjusting your search or category filters.
      </p>
      <button
        onClick={onReset}
...
function SortDropdown({ value, onChange }) {
  ...
  return (
    <div ref={ref} className="relative shrink-0 z-50">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center justify-between w-full sm:w-auto gap-3 bg-white hover:bg-slate-50 border border-slate-200 transition-colors rounded-xl px-5 py-3.5 text-base font-bold text-slate-700 outline-none whitespace-nowrap shadow-sm"
      >
        <div className="flex items-center gap-2">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-2 w-full sm:w-56 bg-white rounded-2xl border border-slate-100 shadow-2xl overflow-hidden z-50"
          >
            <div className="p-2 flex flex-col gap-1">
              {SORT_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 text-sm font-bold transition-all rounded-xl ${
                    value === opt.value
  ...
  const filteredCourses = useMemo(() => {
    ...
    return courses
      .filter((c) => {
        ...
        const matchCat =
          activeCategory === "All" || c.category?.name === activeCategory;
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "rating")
          return avgRating(b.ratingAndReviews) - avgRating(a.ratingAndReviews);
        ...
        <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-4 lg:items-center">
            ...
            <div className="relative w-full lg:max-w-sm shrink-0 group">
              <Search
                size={20}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors pointer-events-none"
              />
              <input
                type="text"
                value={search}
                ...
                className="w-full bg-white border border-slate-200 rounded-full pl-12 pr-10 py-3.5 text-base text-slate-900 font-medium placeholder:text-slate-400 outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 transition-all shadow-sm"
              />
              {search && (
                <button
                  onClick={() => setSearch("")}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700 bg-slate-100 rounded-full p-1 transition-colors"
                >
                  <X size={14} />
                </button>
              )}
                ...
                {allCategoryNames.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 px-6 py-3 rounded-full text-sm font-bold whitespace-nowrap transition-all duration-200 border ${
                        isActive 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {!loading && !error && (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            ...
            {hasActiveFilter && (
              <button
                onClick={() => { setSearch(""); setActiveCategory("All"); setSortBy("popular"); }}
        ...
        {error && (
          <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-3xl border border-rose-100 shadow-sm">
            ...
            <p className="text-slate-500 mt-2 text-lg">We couldn't load the catalog. Please try refreshing.</p>
          </div>
        )}
        ...
        {!loading && !error && filteredCourses.length === 0 && (
          <EmptyState onReset={() => { setSearch(""); setActiveCategory("All"); setSortBy("popular"); }} />
        )}
